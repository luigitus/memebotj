        ...
        try {
            if (data[0].equals("add") && checkPermissions(sender, CommandPower.modAbsolute, CommandPower.modAbsolute)) {
                ...
                if (getChannelHandler().findCommand(data[1]) == -1) {
                    ...
                    getChannelHandler().sendMessage(Memebot.formatText("ADD_COMMAND", getChannelHandler(), sender, this, true, new String[]{newCommand.getCommandName()}, ""), this.getChannelHandler().getChannel());
                    getChannelHandler().getChannelCommands().add(newCommand);
                } else {
                    getChannelHandler().sendMessage(Memebot.formatText("COMMAND_EXISTS", getChannelHandler(), sender, this, true, new String[]{}, ""), this.getChannelHandler().getChannel());
                }
                ...
                if (j != -1) {
                    if (!getChannelHandler().getChannelCommands().get(j).isLocked()) {
                        getChannelHandler().sendMessage(Memebot.formatText("DELCOM_OK", getChannelHandler(), sender, this, true, new String[]{getChannelHandler().getChannelCommands().get(j).getCommandName()}, ""), this.getChannelHandler().getChannel());

                        getChannelHandler().getChannelCommands().get(j).removeDB();
                        getChannelHandler().getChannelCommands().remove(j);
                    } else {
                        getChannelHandler().sendMessage(Memebot.formatText(getChannelHandler().getLocalisation().localisedStringFor("DELCOM_LOCKED"), getChannelHandler(), sender, this, true, new String[]{}, ""), this.getChannelHandler().getChannel());
                    }
                } else {
                    getChannelHandler().sendMessage(Memebot.formatText("DELCOM_NOT_FOUND", getChannelHandler(), sender, this, true, new String[]{}, ""), this.getChannelHandler().getChannel());
                }
                ...
                if (j != -1) {
                    ...
                    if (getChannelHandler().getChannelCommands().get(j).editCommand(data[2], newValue, sender)) {
                        getChannelHandler().sendMessage(Memebot.formatText("EDITCOMMAND_OK", getChannelHandler(), sender, this, true, new String[]{data[1], data[2], newValue}, ""), this.getChannelHandler().getChannel());

                    } else {
                        getChannelHandler().sendMessage(Memebot.formatText("EDITCOMMAND_FAIL", getChannelHandler(), sender, this, true, new String[]{}, ""), this.getChannelHandler().getChannel());
                    }
                }
                ...
                getChannelHandler().sendMessage(Memebot.formatText("SETDATA_OK", getChannelHandler(), sender, this, true, new String[]{}, ""), this.getChannelHandler().getChannel());
                ...
                if (i != -1) {
                    ...
                    if (ch.getCommandName().equals(this.getCommandName())) {
                        getChannelHandler().sendMessage(Memebot.formatText(getChannelHandler().getLocalisation().localisedStringFor("COMMAND_DISABLE_FAILED"), getChannelHandler(), sender, this, false, new String[]{sender.screenName()}, ""), this.getChannelHandler().getChannel());
                    } else {
                        ch.setEnabled(!ch.isEnabled());
                        getChannelHandler().sendMessage(Memebot.formatText(getChannelHandler().getLocalisation().localisedStringFor("COMMAND_TOGGLE"), getChannelHandler(), sender, this, false, new String[]{sender.screenName(), data[1], Boolean.toString(ch.isEnabled())}, ""), this.getChannelHandler().getChannel());
                        ch.writeDB();
                    }
                }
                ...
                if (j != -1) {
                    getChannelHandler().sendMessage(Memebot.formatText(getChannelHandler().getLocalisation().localisedStringFor("COMMAND_TIMES_EXECUTED"), getChannelHandler(), sender, this, false, new String[]{Integer.toString(getChannelHandler().getInternalCommands().get(j).getExecCounter()), getChannelHandler().getInternalCommands().get(j).getOtherData().toString(), getChannelHandler().getInternalCommands().get(j).toString()}, ""), this.getChannelHandler().getChannel());
                }
                ...
                if (Memebot.useWeb) {
                    ...
                    getChannelHandler().sendMessage("Commands: " + output, this.getChannelHandler().getChannel());
                }
            }
        } catch(ArrayIndexOutOfBoundsException e) {
            getChannelHandler().sendMessage(this.getHelptext(), this.getChannelHandler().getChannel());
        } catch(NumberFormatException e) {
            getChannelHandler().sendMessage(Memebot.formatText("COMMAND_LIST_ERROR", getChannelHandler(), sender, this, true, new String[]{}, ""), this.getChannelHandler().getChannel());
            e.printStackTrace();
        }
    }
}
