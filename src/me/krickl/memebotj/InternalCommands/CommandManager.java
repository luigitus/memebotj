		...
		try {
			if(data[0].equals("add")) {
				...
				if ((j = channelHandler.findCommand(data[1])) != -1) {
					...
					if (channelHandler.getChannelCommands().get(j).editCommand(data[2], newValue, sender,
							channelHandler.getUserList())) {
						channelHandler.sendMessage(
								channelHandler.getBuiltInStrings().get("EDITCOMMAND_OK").replace("{param1}", data[1])
										.replace("{param2}", data[2]).replace("{param3}", newValue),
								this.getChannelOrigin());
					} else {
